...
export default function ThemePanel() {
  ...
  const [active, setActive] = useState(0);
	...
	const handleRtlModeCheckboxChange = (isChecked: boolean) => {
		const mode = isChecked ? 'rtl' : '';
		setAppRtlMode(mode);
	};

	const setAppRtlMode = (mode: string) => {
		document.documentElement.setAttribute('dir', mode);
		localStorage.appRtlMode = mode;
		setRtlMode(mode);
		document.dispatchEvent(new Event('theme-reload'));
	};
  ...
  const versions = [
    ...
    { href: 'https://seantheme.com/color-admin/admin/angularjs20/', img: '/assets/img/version/angular20x.jpg' },
  ...
  const getSetting = (key: string): boolean => !!settings?.[key as keyof typeof settings];
	...
	useEffect(() => {
		if (typeof window !== 'undefined' && localStorage) {
			...
			setAppRtlMode(localStorage.appRtlMode || '');
		}
		setTimeout(() => {
      ...
      <div className="theme-panel-content" data-scrollbar="true" data-height="100%">
        ...
        <div className="theme-options">
							...
							<div key={index} className="row mt-10px align-items-center">
								...
								<div className="col-4 d-flex">
									<div className="form-check form-switch ms-auto mb-0">
										...
										/>
										<label className="form-check-label" htmlFor={`app${option}`}></label>
									</div>
								</div>
							</div>
						)
					...
					<div className="row mt-10px align-items-center">
						<div className="col-8 control-label text-dark fw-bold">
							<div className="d-flex">RTL Enabled <span className="badge bg-primary ms-1 py-2px position-relative d-flex align-items-center">NEW</span></div>
						</div>
						<div className="col-4 d-flex">
							<div className="form-check form-switch ms-auto mb-0">
								<input type="checkbox" className="form-check-input" id="appThemeRTLMode" checked={rtlMode === 'rtl'} onChange={(e) => handleRtlModeCheckboxChange(e.target.checked)}/>
								<label className="form-check-label" htmlFor="appThemeRTLMode"></label>
							</div>
						</div>
					</div>
				</div>
